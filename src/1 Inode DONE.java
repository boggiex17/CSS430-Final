...
public class Inode {
    ...
    {   // retrieving inode from disk
        // design it by yourself.
        int blockNum = 1 + iNumber / size;      // retrieve block number

        byte[] iData = new byte[Disk.blockSize]; // Create array to read data
        SysLib.rawread(blockNum, iData);         // Read data for block
        int offset = (iNumber % 16) * iNodeSize; // Calculate offset
        length = SysLib.bytes2int(iData,offset); // read lenght
        offset += 4;                        // int so 4 bytes
        count = SysLib.bytes2short(iData,offset);// read count
        offset += 2;                        // short so 2 bytes
        flag = SysLib.bytes2short(iData,offset); // read flag
        offset += 2;                        // short so 2 bytes

        for(int i = 0; i < directSize; i++)  // loop indirect pointers
        {
            // convert byte data to short and store in direct array
            direct[i] = SysLib.bytes2short(iData, offset);
            offset += 2;
        }
        // convert byte data to short and store in indirect array
        indirect = SysLib.bytes2short(iData,offset);
    }
    ...
    int recordBlock(int index, short offset)
    {
        int block = index / Disk.blockSize; //Divide by 512 to find entry
        if(block < directSize) //Block is direct
        {
    ...
    boolean setBlock(short blockNumber) {
        ...
        if (indirect != -1)                 //
            ...
            indirect = blockNumber;         // set inderect pointer
            byte[] loadData = new byte[Disk.blockSize];

            for(int i = 0; i < (Disk.blockSize/2); i++) {
                // transfer data into byte array
                SysLib.short2bytes((short)-1, loadData, i*2);
            }
            SysLib.rawwrite(blockNumber, loadData);
            return true;
